...
65. [Выводы](#65-выводы)
66. [Список литературы](#66-список-литературы)
...
4. **Помехозащищённость:** сигнал +23 дБ сильнее сигнала MEO-ГНСС (ГЛОНАСС) → порог подавления в 200 раз выше по мощности.
...
*LEO-орбита 1000 км обеспечивает на +23 дБ более сильный сигнал по сравнению с MEO-ГНСС (ГЛОНАСС, ~19 100 км). Это эквивалентно 200-кратному увеличению устойчивости к подавлению. Обратная сторона — более высокий Доплер (±38,6 кГц, 38,6 Гц/с) и короткое время пролёта (~11 мин), что определяет требования к приёмнику (§42).*
...
**1. Повышение мощности сигнала (+23 дБ над ГЛОНАСС)**
Основной фактор. LEO-геометрия даёт мощность сигнала на 200× выше, чем у MEO-ГНСС.
...
$$E_{eclipse} = P_{eclipse} \cdot t_{eclipse} = 118 \times (0{,}34 \times 6305) \approx 253 \text{ кДж} \approx 70 \text{ Вт·ч}$$

Глубина разряда ≤ 80% → необходимая ёмкость:
$$C_{bat} = \frac{E_{eclipse}}{0{,}80} \approx 87{,}5 \text{ Вт·ч}$$
...
$$E_{needed} = P_{eclipse} \cdot t_{eclipse,max} = 118 \times 35 \times 60 \approx 248 \text{ кДж} = 68{,}8 \text{ Вт·ч}$$
...
AURORA EIRP = 7 дБВт (≈ 5 Вт; выводится из бюджета линии §10.2: $P_{rx}$ = −149 дБВт, FSPL = 156 дБ). Поток мощности у поверхности Земли:

$$\text{PFD}_{AURORA} = \text{EIRP} - 10\log(4\pi R^2) = 7 - 10\log(4\pi \times 10^{12}) = 7 - 131 = -124\,\text{дБВт/м}^2$$

Порог ITU-R, ст. 21 / S.1340 для защиты RNSS (полоса 4 МГц): −121,5 дБВт/м². AURORA остаётся **ниже порога** на 2,5 дБ. Для сравнения: PFD GPS Block III ≈ −128 дБВт/м² (EIRP ≈ +30 дБВт, R = 20 200 км). Несмотря на более короткую дистанцию, AURORA не нарушает регуляторный предел, что должно быть подтверждено официальным расчётом EPFD в материалах ГКРЧ.
...
### 65.2 Критические технические риски
...
### 65.3 Рекомендации по дальнейшей разработке
...
### 65.4 Открытые вопросы и план закрытия
