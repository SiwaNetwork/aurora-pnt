...
$$\rho_{max} = \arccos\!\left(\frac{6371}{7371}\cos 10°\right) - 10° \approx 62,9° - 10° = 52,9°$$

**Телесный угол зоны покрытия одного спутника:**
$$\Omega_{cov} = 2\pi(1 - \cos\rho_{max}) \approx 2\pi(1 - 0{,}455) = 3{,}42 \text{ ср}$$

Процент поверхности Земли, покрытый одним спутником:
$$f_{cov} = \frac{\Omega_{cov}}{4\pi} = \frac{1 - \cos\rho_{max}}{2} \approx 27{,}2\%$$

**Среднее число видимых спутников** для полной группировки (300 спутников):
$$\bar{N}_{vis} = N_{sats} \cdot f_{cov} = 300 \times 0{,}272 \approx 81$$

Реально (с учётом неравномерности Walker и широтной зависимости) среднее составляет 28–44 спутника (согласно симуляции).
...
$$f_{IF} = \frac{f_1^2}{f_1^2 - f_2^2} \cdot f_1 - \frac{f_2^2}{f_1^2 - f_2^2} \cdot f_2$$
...
$$\text{FSPL} = 20\log_{10}\!\left(\frac{4\pi d f}{c}\right) = 20\log_{10}\!\left(\frac{4\pi \times 3{,}0 \times 10^6 \times 26{,}5 \times 10^9}{3 \times 10^8}\right) \approx 184 \text{ дБ}$$
...
| Потери в пространстве | −184 дБ |
| Усиление Rx антенны | +30 дБ |
| Потери приёмника | −3 дБ |
| **Уровень сигнала (Rx)** | **−100 дБм** |
| Шумовая температура системы | 300 К → N₀ = −144 дБм/Гц |
| **C/N₀** | **44 дБ-Гц** |

### 9.4 Точность измерения дальности ISL

**Кодовый режим (BPSK, 10 Мчип/с):**
$$\sigma_{range,code} = \frac{c}{f_{chip}} \cdot \frac{1}{\sqrt{2 \cdot C/N_0 \cdot T_{int}}} \approx \frac{30 \text{ м}}{\sqrt{2 \times 10^{4{,}4} \times 0{,}1}} \approx 0{,}10 \text{ м}$$

**Фазовый режим (несущая):**
$$\sigma_{range,phase} = \frac{\lambda}{2\pi} \cdot \frac{1}{\sqrt{C/N_0 \cdot T_{int}}} = \frac{0{,}011}{2\pi\sqrt{2{,}5 \times 10^5}} \approx 0{,}007 \text{ м}$$
...
AURORA с 300 спутниками и N_vis ≈ 36 обеспечивает избыточность, необходимую для RAIM.
...
- Фаза 4 (N_vis ≈ 36): доступность > 99,999%
...
Перевод на переходную орбиту с апогеем 1000 км и перигеем ≈ 250 км:

$$\Delta V_{deorbit} = v_{circ}(1000) - v_{perigee}(250) \approx 7350 - 7620 \approx -270 \text{ м/с}$$

(на деле нужен тормозной импульс ~100–120 м/с для перевода перигея на 250 км).

Затем атмосфера понизит апогей, и через 2–5 лет КА войдёт в атмосферу.

**Для немедленного входа** (100 лет → < 5 лет):

Снижение орбиты с 1000 км до 200 км:
$$\Delta V_{total} \approx 120 \text{ м/с}$$
...
- Случайная геометрия из Walker-созвездия (N_vis = 36)
...
Для AURORA (r = 7371 км + 1000 км = 7371 км... нет, $r = 6371 + 1000 = 7371$ км):
...
**Вывод:** у LEO-спутников (1000 км) гравитационный эффект слабее доплеровского, поэтому суммарная поправка отрицательна (часы на спутнике «торопятся»). Для GPS (MEO) — наоборот.
...
| N_vis (средн.) | 6–8 | 36–42 | AURORA (5–6× больше) |
...
$$\mathrm{LCC} = \mathrm{NRE} + \sum_{n=1}^{300} T_1 n^{\log_2 b} + C_{launch} + C_{ground} + \mathrm{OPEX}\cdot Y$$

Кривая обучения Райта b = 0,85, T₁ = 4,0 $M (первый КА).

### 51.2 Бюджет (оценка)

| Статья | Стоимость |
|---|---|
| NRE (проектирование + квалификация) | 120 $M |
| Серия 300 КА (кривая обучения) | 408,9 $M |
| Запуски (25 × 12 $M) | 300 $M |
| Наземный сегмент (CAPEX) | 80 $M |
| OPEX за 7 лет | 280 $M |
| **LCC 7 лет** | **1188,9 $M (≈ 107 млрд ₽)** |
| **LCC 15 лет** | **1508,9 $M** |
| Стоимость на 1 КА | 3,96 $M |
...
⚠️ **Выявленное ограничение:** при 5 станциях TT&C доступно ~600
контактов/сут против требуемых ~1200 для 300 КА (дефицит −50 %).
Требуется расширение сети TT&C до ~10 станций либо снижение частоты
контактов на КА. Зафиксировано как проектное ограничение для следующей итерации.
...
5. **Целостность** (RAIM):
   - При N_vis ≈ 36 RAIM доступен с вероятностью > 99,999%
...
### 55.2 Критические технические риски
...
### 55.3 Рекомендации по дальнейшей разработке
