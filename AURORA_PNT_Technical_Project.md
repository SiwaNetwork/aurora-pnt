...
В документе изложены проектные решения по системе AURORA PNT — низкоорбитальной группировке навигации, позиционирования и синхронизации. Для каждой подсистемы приведены расчётные соотношения, результаты моделирования и обоснование принятого варианта: построение группировки, навигационный сигнал, бортовая шкала времени, межспутниковые линии, наземный сегмент и пользовательская аппаратура.
...
28. [Временна́я служба и шкала времени AURORA](#28-временная-служба-и-шкала-времени-aurora)
...
66. [Выводы](#66-выводы)
67. [Список литературы](#67-список-литературы)
...
Система AURORA PNT разрабатывается как низкоорбитальное дополнение и, в перспективе, замена MEO-навигации, с улучшенными характеристиками сигнала, синхронизации и геометрии покрытия.
...
### 3.3 Наземный сегмент (МКС AURORA)
...
![Системная концепция AURORA PNT](results/system_concept/concept_system_overview_phase4.png)
...
![Сценарии применения AURORA PNT](results/system_concept/concept_service_scenarios_phase4.png)
...
**Рисунок 3.3 — AURORA LEO vs MEO-ГНСС (ГЛОНАСС): преимущество сигнала**

![AURORA LEO vs ГЛОНАСС (MEO): преимущество сигнала](results/system_concept/concept_leo_vs_meo_phase4.png)
...
![Поток сигнала AURORA PNT](results/system_concept/concept_signal_flow_phase4.png)
...
| Параметр | ГЛОНАСС (24 спутника, 19 100 км) | AURORA Ф4 (300 спутников, 1000 км) |
|---|---|---|
...
![Частотный план AURORA](results/freq_plan/freq_plan_phase3.png)
...
- Мощность в точке пользователя AURORA L1: ≈ −107 дБм у Сервиса Б (выделенная полоса) и −119 дБм у Сервиса А (RNSS), против −130 дБм MEO-ГНСС (§65)
...
### 6.7 Структура навигационного сообщения AURORA (ANAV)

Навигационное сообщение LEO-системы имеет критическое отличие от MEO: **эфемериды меняются на порядок быстрее**. Орбитальный период AURORA ≈105 мин; через 10 мин спутник смещается на ~4 400 км — ошибка при использовании устаревших эфемерид растёт как t².
...
Для AURORA (зенит): $\Delta = 52{,}6 - 25 = 27{,}6$ дБ → помехе нужна мощность в $10^{2{,}76} \approx 575$ раз больше, чем для подавления GPS на той же высоте.
...
|---|---|---|---|---|
| AURORA МКС (21 ст.) | 21 | 2,0 | 0,5 | 0,30 |
| GPS (18 ст., глоб.) | 18 | 6,0 | 0,1 | 0,30 |
| AURORA МКС + ИСЛ | 21 | 24,0 | 0,0 | 0,10 |

**Результаты ОО (1σ):**

| Сеть | R (м) | A (м) | C (м) | 3D (м) | UERE (м) |
|---|---|---|---|---|---|
| AURORA МКС (21 ст.) | 0,10 | 1,02 | 0,48 | 1,12 | 0,33 |
| GPS (глоб., 18 ст.) | 0,06 | 0,52 | 0,25 | 0,58 | 0,18 |
| AURORA МКС + ИСЛ | 0,02 | 0,20 | 0,09 | 0,22 | 0,07 |
...
AURORA с 300 спутниками и N_vis ≈ 14 (7–22 в зависимости от широты, см. §46.2) обеспечивает избыточность ($m \geq 7 \gg 5$ почти везде), необходимую для RAIM/ARAIM.
...
При дальности 10 км от джаммера:
- GPS: $P_{jam}$ ≈ 1 Вт
- AURORA: $P_{jam}$ ≈ 575 Вт (превышение в 575 раз)
...
### 17.2 Преимущество AURORA в борьбе с многолучёвостью
...
### 24.1 Роль СДКМ в архитектуре AURORA
...
- **По частоте:** $\pm 38{,}6$ кГц (L1) — в 8× шире, чем для GPS
...
## 28. Временна́я служба и шкала времени AURORA

### 28.1 Системная шкала времени AURORA Time (AT)

AURORA Time (AT) — собственная атомная шкала системы, физически реализуемая группой активных водородных мазеров Ч1-1008 (ВРЕМЯ-Ч) наземного эталона и связанная с UTC(SU):
...
```
UTC(SU) ──[AURORA LPT, радио]──► узел-источник SHIWA ──[SHIWA TIME mesh]──► потребители
```
...
**Сквозной бюджет AURORA + SHIWA TIME:**
...
(AURORA-ТЭО-001): оценка рынка, коммерческая модель, выручка и срок окупаемости выходят за
...
Орбита AURORA (1000 км / 75°) пересекает внутренний пояс протонов Ван Аллена. Интенсивность потока протонов с энергией > 10 МэВ составляет ≈ 5×10¹² п/см²/год (AP8-MIN). Без экранирования доза ионизации (TID) достигает 80 кРад/год (Si).
...
Для AURORA ($v = 7354$ м/с): $-0{,}3008$ ppb ≈ −26,0 мкс/день.
...
### 36.3 Время сходимости AURORA vs MEO-ГНСС
...
|---|---|---|---|
| AURORA FOC | 14 (до 22) | Да | 1–3 мин |
| AURORA IOC (Ф2) | 5 | Да | ~5–10 мин |
| GPS only | 9 | Да | ~2–5 мин |
| GPS (стандарт, 1 сп.) | 8 | Нет | 20–40 мин |

**Вывод:** AURORA обеспечивает быструю сходимость PPP (1–3 мин против 20–40 мин у одиночного MEO) за счёт большого числа спутников с быстро меняющейся геометрией — это одно из ключевых преимуществ LEO.

### 36.4 Геометрия LEO vs MEO

| Параметр | AURORA LEO (1000 км) | ГЛОНАСС MEO (19 100 км) |
|---|---|---|
...
### 37.2 Сравнение AURORA / GPS / Galileo
...
| **AURORA L1/L5** | **0,45 м** | **0,50 м** | **2,5 м** | **0,70 м** |

По точности **одной псевдодальности** AURORA (UERE dual ≈ 0,7 м) сопоставима с ГЛОНАСС и
несколько уступает GPS/Galileo. Это плата за низкую орбиту (сложнее орбитальное определение,
быстрая динамика) и бортовой CSAC вместо PHM/Cs у MEO. Конкурентное преимущество AURORA лежит
**не в точности дальности**, а в геометрии комбинированного режима, мощности сигнала и скорости
PPP-сходимости (см. §41). Тропосфера и многолучёвость у AURORA при этом ниже благодаря высоким
углам возвышения LEO, что частично компенсирует орбитально-часовой вклад.

### 37.3 Бюджет ошибок AURORA (dual-freq)
...
Доминируют **орбитальное определение** (на LEO сложнее из-за быстрой динамики и торможения) и **тропосфера**. Шум приёмника и многолучёвость у AURORA, наоборот, снижены за счёт мощного сигнала (+23 дБ) и высоких углов возвышения. Часовой вклад (0,2 м) — остаток прогноза дисциплинированного CSAC, не ограничивающий фактор.

![URE/SISA/UERE сравнение](results/signal_quality/signal_ure_budget_phase4.png)

![Бюджет UERE AURORA](results/signal_quality/signal_uere_breakdown_phase4.png)
...
## 41. Сравнение AURORA PNT с аналогами
...
| N_vis (средн.) | 6–8 | 14 (7–22 по широте) | AURORA (≈2× в среднем, до 3× в высоких широтах) |
...
| UERE L1-only | 3,5 м | 2,5 м | AURORA |
| PDOP p95 (комбинир. LEO+ГЛОНАСС) | 2,0 | 1,67 | AURORA (геометрия) |
| PPP сходимость | ~30 мин | 1–3 мин | AURORA (≈15×) |
| Мощность сигнала (Сервис Б) | ~−130 дБм | −107 дБм | AURORA Б (+23 дБ, ×200); Сервис А −119 дБм (+11 дБ) |
| Аутентификация | Нет | TESLA MAC | AURORA |
| Временна́я точность | 10 нс (цель) | 5 нс | AURORA (2×) |
...
**Синхронизация:** ГЛОНАСС-K2 достигает точности шкалы времени ~10 нс относительно UTC(SU); AURORA с бортовым CSAC и ISL-синхронизацией нацелен на **5 нс** — в 2× лучше.

#### Общая таблица MEO GNSS vs AURORA
...
**Вывод:** по точности одной псевдодальности AURORA сопоставима с MEO GNSS (несколько уступая GPS/Galileo), а в автономном режиме её геометрия (PDOP p95 ≈ 5) даже хуже из-за специфики LEO. Реальные преимущества AURORA — в **комбинированном режиме** (LEO+ГЛОНАСС → PDOP 1,67, CEP ≈ 1 м, PPP-сходимость 1–3 мин), **мощности сигнала** (+23 дБ: помехозащита и работа в помещениях), **арктическом покрытии** (75°) и **аутентификации** (TESLA MAC). AURORA позиционируется как суверенное LEO-усиление ГЛОНАСС, а не самостоятельная замена MEO.
...
**Ключевое отличие:** Xona сфокусирован на рынке США и союзников, AURORA — на суверенном российском сегменте и арктических регионах. Наклонение 75° у AURORA даёт существенное преимущество над Арктикой.
...
![AURORA vs ГЛОНАСС детально](results/competitor_analysis/comp_aurora_vs_glonass_phase4.png)
...
![Доплеровский профиль AURORA vs MEO](results/user_segment/user_doppler_phase4.png)
...
**Численные результаты (бюджет AURORA C/N₀ = 52,6 дБ-Гц):**
...
![Маска OOB AURORA L1](results/itu_coordination/itu_oob_mask_phase4.png)
...
Это делает программу AURORA конкурентоспособной без западного импорта критичной
элементной базы.
...
Программа AURORA PNT — 13 фаз с янв. 2026 по янв. 2043 (204 мес. = 17 лет
...
**Вывод по соответствию:** AURORA PNT соответствует всем
...
Координатная основа AURORA PNT — государственная геодезическая система **ПЗ-90.11** («Параметры Земли 1990», уточнение 2011 г.), та же, что у ГЛОНАСС. Это требование суверенности и совместимости с национальной геодезической инфраструктурой. Положения КА, эфемериды и решение пользователя выражаются в ПЗ-90.11; шкала времени — UTC(SU)/AURORA System Time (§28), парная пространственному датуму.
...
AURORA формирует с одной группировки (1000 км) **два независимых навигационных сервиса** с разными полосами и регуляторными режимами по плотности потока мощности (ПФП). Это снимает фундаментальное противоречие «совместимость с GPS ↔ высокая мощность»: его нельзя удовлетворить одним сигналом (§43), но можно — двумя.

| | **Сервис А — открытый** | **Сервис Б — защищённый** |
|---|---|---|
| Полоса | L1/L5 RNSS (наложение на GPS/Galileo) | выделенная L-полоса AURORA (первичный статус) |
| Режим ПФП | маска МСЭ-R −121,5 дБВт/м² | собственная полоса (как Xona/Iridium) |
| Преимущество по сигналу | **+11 дБ (×11)** | **+23 дБ (×200)** |
| Приёмник | GNSS-совместимый | специальный приёмник AURORA |
...
| **AURORA Сервис А** | **+11 дБ (×11)** | 1000 км | RNSS (совместимый) |
| Xona PULSAR | +20 дБ (×100) | ~550 км | своя L/C-полоса |
| **AURORA Сервис Б** | **+23 дБ (×200)** | 1000 км | своя L-полоса |
| Iridium STL | +30 дБ (×1000) | 780 км | MSS-полоса |

Сервис Б ставит AURORA в один ряд с Xona и Iridium STL по силе сигнала и помехозащите, **не снижая орбиту** (выигрыш в покрытии/числе КА). При этом конкуренты дают **либо** мощный сигнал в своей полосе (Xona/Iridium), **либо** GNSS-совместимость (MEO) — **AURORA уникально даёт оба** сервиса с одной группировки.
...
2. **Мощность сигнала** на +23 дБ выше ГЛОНАСС (MEO) делает AURORA:
