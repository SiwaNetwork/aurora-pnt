...
| Аутентификация (TESLA MAC) | 128 бит | HMAC-SHA256; накладные расходы 4,27 бит/с |
...
## 16. Аутентификация навигационного сигнала (TESLA MAC)

### 16.1 Структура TESLA
...
3. В момент $t_{i+N}$ раскрывается $k_i$; получатель проверяет MAC
...
**Ограничение:** задержка верификации = период раскрытия (30 с). Данный метод аутентифицирует прошлые сообщения, а не текущие.

### 16.2 Устойчивость к спуфингу

Вероятность успешного спуфинга при TESLA:
- Без знания будущего ключа: $P_{spoof} = 2^{-128}$ (криптостойкость SHA-256)
...
| Кибератака на TESLA MAC / НС | Низкая | Высокое | HMAC-SHA256 128 бит, смена ключа 30 с, аппаратный HSM на МКС |
