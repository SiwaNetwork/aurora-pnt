...
|---|---|---|
| OP-01 | Радиационная стойкость CSAC (SA.45s) 20 крад при 1000 км / 7 лет требует ~7 мм Al (запас 1,2×) | Локальное экранирование или подтверждение дозы секторным анализом (§32) |
| OP-02 | Орбитальное определение LEO — доминанта UERE (~0,45 м) | Уточнить достижимую точность POD с ISL + наземной сетью (§13, §47); фазовая ISL-дальнометрия |
...
| OP-04 | Модуль `agps_server` (сокращение холодного TTFF) не реализован | Реализовать (роадмап §22) |
