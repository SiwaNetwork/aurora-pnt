...
Система AURORA PNT разрабатывается как НОО-дополнение и долгосрочная замена MEO-навигации с радикально улучшенными характеристиками сигнала, синхронизации и геометрии покрытия.
...
*Цепочка: Cs-стандарт (< 0,01 нс/6ч) → навигационный передатчик (L1 5 Вт / L5 3 Вт) → сигнал через 1000 км (FSPL −159 дБ, задержка 3,3 мс) → приёмник (≥ 45 дБ·Гц, 238 каналов) → Kalman-фильтр с IF-комбинацией L1+L5 → PNT решение < 0,5 м / < 5 нс UTC(SU).*
...
**Калибровочные точки симуляции (среднее N_vis по широтам):**
...
| Фаза | Сп. | Покрытие (глоб.) | PDOP p50 | PDOP p95 | N_vis (ср.) |
|---|---|---|---|---|---|
...
$$\alpha_1 = \frac{f_1^2}{f_1^2 - f_2^2} = \frac{1575{,}42^2}{1575{,}42^2 - 1176{,}45^2} \approx 2{,}546$$
$$\alpha_2 = \frac{f_2^2}{f_1^2 - f_2^2} \approx 1{,}546$$
...
|---|---|---|---|
| Cs (цезий, первичный) | 5 × 10⁻¹² | −0,50 | Главный КА, МКС |
| Rb (рубидий, вторичный) | 3 × 10⁻¹¹ | −0,50 | Большинство КА |
| OCXO (кристальный, термо.) | 1 × 10⁻¹⁰ | −1,00 | Резервный, абонентский |
| TCXO (кристальный, темп.-комп.) | 1 × 10⁻⁹ | −1,00 | Дешёвые пользовательские приёмники |
...
$$\sigma_t(100) = 3 \times 10^{-11} \times \sqrt{100} \times 10^9 = 3 \times 10^{-11} \times 10 \times 10^9 \approx 300 \text{ пс}$$

За 1 час (3600 с):
$$\sigma_t(3600) = 3 \times 10^{-11} \times \sqrt{3600} \times 10^9 = 3 \times 10^{-11} \times 60 \times 10^9 \approx 1{,}8 \text{ нс}$$
...
```
UTC(SU) [водородный мазер МКС]
    │  наземная линия (~0.01 нс)
    ▼
Cs-эталон (главный КА)
    │  ISL код: 0.30 м → 1.0 нс
    ▼
Rb 1-хоп (через ISL)
    │  ISL × 3 хопа
    ▼
...
$$\sigma_{IF} = \sqrt{\alpha_1^2 + \alpha_2^2} \cdot \sigma_{single} = \sqrt{2{,}546^2 + 1{,}546^2} \cdot \sigma \approx 3{,}0 \cdot \sigma$$
...
AURORA с 300 спутниками и N_vis ≈ 11 (7–22 в зависимости от широты, см. §46.2) обеспечивает избыточность ($m \geq 7 \gg 5$ почти везде), необходимую для RAIM/ARAIM.
...
- Фаза 4 (N_vis ≈ 11, 7–22 по широте): доступность > 99,999% (избыточность $m-4 \geq 3$ почти всегда)
...
- Прямое сравнение с водородным мазером на станции МКС: $ \sigma_{comp} \approx 0{,}1$ нс
...
$$\delta_{rel} = -\frac{2 \mathbf{R} \times \mathbf{v}}{c^2} \approx -207{,}4 \text{ нс (AURORA 1000 км)}$$
...
| Фаза | Cп. | N_vis | PDOP | UERE (L1+L5) | H-95% | V-95% |
|---|---|---|---|---|---|---|
| 0 | 3 | 0,5 | 50 | — | N/A | N/A |
...
### 31.2 Сравнение с аналогами

| Система | Число КА | Орбита | Бюджет ($B) | Дата начала |
|---|---|---|---|---|
| GPS Block III | 10 | MEO 20 200 км | 5,5 | 2018 |
| Galileo (полная) | 30 | MEO 23 222 км | 10 | 2011 |
| ГЛОНАСС (восст.) | 24 | MEO 19 100 км | 2,5 | 2001 |
| OneWeb | 648 | LEO 1200 км | 3,4 | 2019 |
| **AURORA PNT** | **300** | **LEO 1000 км** | **~2,8** | **2026** |

### 31.3 Коммерческие применения

| Применение | Потенциальная выручка |
|---|---|
| LPT сервис телекоммуникациям | $50M/год |
| Точная навигация (сельское хоз.) | $30M/год |
| Морские суда (РФ + СМП) | $20M/год |
| Авиация (РФ) | $15M/год |
| Экспорт сервиса (страны партнёры) | $100M/год |
| **Итого потенциальная выручка** | **$215M/год** |
...
| Многолучёвость | 0,200 | 57,4% |
| Шум приёмника | 0,150 | 32,2% |

Доминирующие источники — многолучёвость и шум приёмника. Оба улучшаются специализированными антеннами и алгоритмами.
...
| UERE dual (м) | 0,461 | 0,520 | 0,363 | 0,420 | **0,279** |
...
$$f_{vis} = \frac{1 - \cos\rho}{2}, \quad \rho = \arcsin\!\left(\frac{R_E}{R_E + h}\right) \approx 52{,}9°$$
$$N_{vis} \approx 300 \times f_{vis} \times 2 \approx 36 \text{ спутников (L1)}$$

Для L1+L5 режима: **72 канала** активного слежения + резерв поиска ≈ **238 каналов итого** (геодезический класс).
...
Атомные часы (Cs+Rb+OCXO+обвязка) — отдельная закупная позиция:
...
| **Бортовой стандарт частоты** (300 × 5 млн ₽, РФ) | **1 500** | **17** |
...
⚠️ **Выявленное ограничение и план закрытия:** при 5 станциях TT&C доступно
...
| SAT-04 | Стабильность Cs-часов τ=100 с (ADEV) | ≤ 3·10⁻¹¹ |
...
1. **Созвездие 300 КА Walker 300/15/1** на орбите 1000 км / 75° обеспечивает:
   - 100% глобальное покрытие при N_vis ≥ 4
...
3. **Точность синхронизации:**
   ...
   - OCXO через 6 ISL-хопов: 2,58 нс
...
4. **Двухчастотная коррекция L1+L5** устраняет ионосферу, обеспечивая:
   ...
   - H-95% ≈ 3,4 м (Фаза 4, глобально)

5. **Целостность** (RAIM):
   - При N_vis ≈ 11 (7–22 по широте) RAIM доступен с вероятностью > 99,999%
...
| Временно́й сдвиг Rb > 100 нс/6ч | Низкая | Среднее | Cs-резерв, частые обновления |
...
1. **Фаза 0 (2026):** фокус на летной квалификации ISL Ka-диапазона и Cs-стандарта
...
**Дата:** Май 2026  
**Проект:** LEOPath / ShiwaNetwork  
**Статус:** Технический проект (ТП), готов к использованию как основа ТЗ
