...
Развитие проекта — **распределённый (ансамблевый) эталон времени на группировке**. Вместо дорогого атомного стандарта на каждом из 300 КА протокол SHIWA TIME-Space синхронизирует созвездие по ISL в единую **составную (ансамблевую) шкалу**: дешёвые чип-цезиевые CSAC-терминалы на всех КА дисциплинируются по немногим якорям space-Rb (~15) и наземному H-мазеру (§8).

**Принцип составной шкалы.** По теории композитных часов системное время вычисляется обратно-дисперсионным взвешиванием всех бортовых стандартов:

$$\sigma_{ens}^2(\tau) = \frac{1}{\dfrac{N_{терм}}{\sigma_{CSAC}^2(\tau)} + \dfrac{N_{якор}}{\sigma_{Rb}^2(\tau)}}$$

Ансамбль устойчивее любого отдельного дешёвого стандарта: при $N_{терм}=300$ CSAC и $N_{якор}=15$ space-Rb системная шкала достигает **σ_y(1 с) ≈ 2,6 × 10⁻¹²** — уровня space-Rb, хотя дорогой стандарт стоит лишь на 15 якорях. CSAC-терминалу достаточно удерживать шкалу между ISL-обновлениями (~10 с), а долговременную стабильность задаёт ансамбль якорей.

**Обработка асимметрии ISL.** В отличие от наземного «счастливого пакета», на ISL задержка детерминирована, но асимметрична (движение КА, релятивистика, Саньяк, разные пути «туда/обратно»). SHIWA TIME-Space снимает асимметрию двусторонним обменом метками (TWSTT, §8.3): симметричная часть задержки сокращается, остаточная — корректируется по эфемеридам (релятивистский член §28.3, Саньяк). Распределённый слой консенсуса (как в наземном варианте, §28.4) выбирает вклад каждого узла по измеренной стабильности, без единой точки отказа.

**Экономический эффект.** При относительной стоимости space-Rb ≈ 10× CSAC стоимость часового сегмента снижается с 3000 ед. (space-Rb на всех 300 КА) до **450 ед.** (300 CSAC + 15 space-Rb) — **на ≈ 85 %**, при сохранении системной шкалы уровня space-Rb. Это и есть цель развития: суверенный распределённый эталон на группировке без дорогих стандартов на каждом аппарате.

Модель — `aurora/pnt/ensemble_timescale.py` (CLI `ensemble-timescale`).
