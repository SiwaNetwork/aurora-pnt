...
TP_ONLY = [
    # CSAC по физике — чип-масштабный цезий (Cs-133/CPT), поэтому «Cs»/«цезий» сам
    # по себе НЕ запрещён. Ловим только маркеры СТАРОЙ схемы: цезий как отдельный
    # лучевой прибор на борту, Cs-мастер на земле (должен быть H-мазер).
    ("1× Cs, 2× Rb", "старая часовая схема (Cs+2Rb на КА)"),
    ("Cs-мастер", "старый Cs-мастер (на земле должен быть H-мазер Ч1-1008)"),
