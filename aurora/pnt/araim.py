...
защиты AURORA PNT методом разделения решений (solution separation).

Вычисляет:
- Геометрию «всех видимых» спутников плотного LEO-созвездия AURORA (~12 в зоне)
...
# ── Параметры системы AURORA (плотное LEO, хорошая геометрия) ─────────────────
