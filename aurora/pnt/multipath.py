...
def _plot_mp_by_environment(summary: List[Dict], output_dir: str, label: str) -> None:
    ...
    ax.bar_label(b1, fmt="%.2f m", padding=2, fontsize=7)
    ax.bar_label(b2, fmt="%.2f m", padding=2, fontsize=7)
    ax.set_xticks(x)
    ax.set_xticklabels(envs, rotation=20, ha="right", fontsize=9)
    ax.set_ylabel("Multipath error 1-sigma (m)")
    ax.set_title("Multipath Error by Environment")
    ax.legend()
    ...
    ax2.bar_label(b3, fmt="%.1f m", padding=2, fontsize=7)
    ax2.bar_label(b4, fmt="%.1f m", padding=2, fontsize=7)
    ax2.set_xticks(x)
    ax2.set_xticklabels(envs, rotation=20, ha="right", fontsize=9)
    ax2.set_ylabel("CEP 50% (m)")
    ax2.set_title("CEP by Environment (AURORA L1+L5 vs GPS L1)")
    ax2.legend()
    ax2.grid(axis="y", alpha=0.3)

    fig.suptitle(f"AURORA PNT — Multipath Analysis [{label}]", fontsize=12)
    plt.tight_layout()
...
def _plot_uere_breakdown_env(results: List[Dict], output_dir: str, label: str) -> None:
    ...
    x = np.arange(len(envs))
    ax.bar(x, non_mp, label="Non-multipath UERE", color="#74b9ff", edgecolor="white")
    ax.bar(x, mps, bottom=non_mp, label="Multipath", color="#e17055", edgecolor="white")
    for xi, u in zip(x, ueres):
        ax.text(xi, u + 0.05, f"{u:.2f}m", ha="center", fontsize=8, fontweight="bold")
    ax.set_xticks(x)
    ax.set_xticklabels(envs, rotation=20, ha="right", fontsize=9)
    ax.set_ylabel("UERE 1-sigma (m)")
    ax.set_title(f"AURORA L1+L5 UERE Stack by Environment [{label}]")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)
    plt.tight_layout()
