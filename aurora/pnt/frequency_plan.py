...
def _plot_interference_summary(interference: Dict, output_dir: str, label: str) -> None:
    ...
    ax.axhline(0.25, ls="--", color="#e17055", lw=1.5, label="Порог МСЭ-R 0,25 дБ")
...
def _plot_doppler(doppler: Dict, altitude_m: float, output_dir: str, label: str) -> None:
    ...
    for ax_idx, (band_name, d) in enumerate(doppler.items()):
        ...
        ax.set_ylabel("Доплеровский сдвиг (кГц)")
        ax.set_title(f"{band_name} ({d['center_mhz']:.2f} МГц)\nМакс. скорость: "
                     f"{d['doppler_rate_hz_s']:.1f} Гц/с")
        ax.grid(alpha=0.3)
        ax.set_xlim(0, 90)
