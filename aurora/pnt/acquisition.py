...
def _plot_doppler_comparison(results: Dict, output_dir: str, label: str) -> None:
    ...
    ax.bar_label(bars, fmt="%.1f kHz", padding=3, fontsize=9)
    ax.set_ylabel("Max Doppler shift (kHz)")
    ax.set_title("Max Doppler Shift")
    ax.grid(axis="y", alpha=0.3)

    ax2 = axes[1]
    bars2 = ax2.bar(names, dr, color=colors, edgecolor="white", width=0.5)
    ax2.bar_label(bars2, fmt="%.1f Hz/s", padding=3, fontsize=9)
    ax2.set_ylabel("Doppler rate (Hz/s)")
    ax2.set_title("Max Doppler Rate of Change")
    ax2.grid(axis="y", alpha=0.3)

    fig.suptitle(f"AURORA vs GPS Doppler [{label}]", fontsize=12)
    plt.tight_layout()
...
def _plot_ttff_comparison(results: Dict, output_dir: str, label: str) -> None:
    ...
    w = 0.25
    b1 = ax.bar(x - w,   cold, w, label="Cold start", color="#e17055", edgecolor="white")
    b2 = ax.bar(x,       warm, w, label="Warm start", color="#fdcb6e", edgecolor="white")
    b3 = ax.bar(x + w,   hot,  w, label="Hot start",  color="#00b894", edgecolor="white")
    ax.bar_label(b1, fmt="%.0f s", padding=2, fontsize=8)
    ax.bar_label(b2, fmt="%.0f s", padding=2, fontsize=8)
    ax.bar_label(b3, fmt="%.1f s", padding=2, fontsize=8)
    ax.set_xticks(x)
    ax.set_xticklabels(names, fontsize=9)
    ax.set_ylabel("TTFF (s)")
    ax.set_title(f"Time-To-First-Fix [{label}]")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)
    plt.tight_layout()
...
def print_acquisition_summary(label: str, result: Dict) -> None:
    ...
    print(f"\n{sep}")
    print(f"  Signal Acquisition & TTFF -- {label}")
    print(f"  RX clock: {result['rx_clock_ppm']} ppm  "
          f"Parallel channels: {result['parallel_ch']}")
    print(sep)
    ...
    for sig_name, r in result["results"].items():
        ...
        s = r["search"]
        print(f"  [{sig_name}]")
        print(f"    Max Doppler:       {r['max_doppler_hz']/1000:>8.1f} kHz")
        print(f"    Doppler rate:      {r['doppler_rate_hz_s']:>8.1f} Hz/s")
        print(f"    Freq bins:         {s['freq_bins']:>8}")
        print(f"    Code bins:         {s['code_bins']:>8}")
        print(f"    Total search cells:{s['total_search_cells']:>8,}")
        print(f"    Coh. integration:  {t['coh_integration_ms']:>8.2f} ms")
        print(f"    Dwell per cell:    {t['dwell_ms']:>8.2f} ms")
        print(f"    TTFF cold start:   {t['ttff_cold_s']:>8.1f} s")
        print(f"    TTFF warm start:   {t['ttff_warm_s']:>8.1f} s")
        print(f"    TTFF hot start:    {t['ttff_hot_s']:>8.2f} s")
        print()
    print(sep)
