"""
Геодезическая система отсчёта АВРОРА и параметры вращения Земли (EOP).
...
с ITRF2008 на уровне миллиметров. Модель:
  ...
  1. 7-параметрическое преобразование Гельмерта ПЗ-90.11 ↔ ITRF2008 (↔ WGS-84):
     ...
     вращения и масштаба — т.е. координатная основа АВРОРА совместима с ГЛОНАСС,
...
def run_reference_frame_analysis(output_dir: str, label: str) -> Dict:
    ...
    fig.suptitle(f"АВРОРА — геодезическая система отсчёта (ПЗ-90.11) и EOP [{label}]",
