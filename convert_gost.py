...
def add_abstract(doc):
    ...
    abstract = (
        "Настоящий технический проект описывает систему AURORA PNT — автономную "
        "низкоорбитальную спутниковую систему позиционирования, навигации и синхронизации "
        "(LEO PNT). Документ содержит аналитические модели и результаты численного "
        "моделирования по всем подсистемам: созвездие Walker 300/15/1 на орбите 1000 км, "
        "покрытие и геометрия (PDOP), навигационный сигнал L1/L5, синхронизация "
        "(Allan deviation, ISL timing transfer), межспутниковые каналы Ka-диапазона, "
        "ионосферная коррекция, орбитальное определение, RAIM, энергетика, масса, "
        ...
        "Ключевые слова: LEO, PNT, GNSS, Walker, ISL, PDOP, UERE, RAIM, Allan deviation, "
        "AURORA, ГОСТ 7.32, спутниковая навигация."
    )
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p.paragraph_format.first_line_indent = INDENT
